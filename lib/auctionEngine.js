...
function markReassignedAllocations(allocationRows, previousAllocations = []) {
  ...
  for (const allocation of previousAllocations || []) {
    previousSignatures.set(`${allocation.member_id}:${allocation.item_id}`, JSON.stringify(allocation.page_assignments || []));
  }

  return allocationRows.map((row) => {
    const currentSignature = JSON.stringify(row.page_assignments || []);
