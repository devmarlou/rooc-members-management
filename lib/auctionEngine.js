...
export async function startAuction(supabase, payload) {
  ...
  const activeAuction = await selectMaybeSingle(
    supabase.from("auctions").select("id").eq("type", type).in("status", OPEN_AUCTION_STATUSES).limit(1)
  );
  if (activeAuction) throw new Error(`Finish the open ${type === "league_prize" ? "League Prize" : "GL/WoE"} auction before starting another one.`);
  ...
  if (type === "league_prize") {
    ...
    if (!sourceGlAuction) throw new Error("Create and lock a GL/WoE auction list before starting League Prize.");
    if (sourceGlAuction.status !== "locked") throw new Error("Lock-in the GL/WoE list before starting League Prize.");

    const sourceQueueResult = await supabase
      .from("auction_queue")
    ...
    orderedMemberIds = (sourceQueueResult.data || [])
      .filter((row) => row.status !== "cant_pay")
      .map((row) => row.member_id);
    queueMemberIds = new Set(orderedMemberIds);
  }

  const allocationResult = buildAllocationRows({ context, inventoryByItemId, queueMemberIds, orderedMemberIds });
...
export async function recalculateActiveAuction(supabase, auctionId = null) {
  const activeAuction = await selectOpenAuction(supabase, auctionId);
  if (!activeAuction) throw new Error("There is no active auction.");
  if (activeAuction.status === "locked") throw new Error("This auction list is locked.");
