...
function buildAllocationRows({ context, inventoryByItemId, excludedMemberIds = new Set(), queueMemberIds = null }) {
  const candidates = context.rotation
    ...
    .map((entry) => {
      ...
      const canReceive = context.items.some((item) => {
        const assignedQuantity = 0;
        return (received[item.item_key] || 0) + assignedQuantity < context.capResolver.capFor(entry.member_id, item);
      });
      return { ...entry, received, canReceive };
    })
  ...
  for (const item of context.items) {
    ...
    while (remaining > 0) {
      ...
      for (const entry of candidates) {
        ...
        const capacity = context.capResolver.capFor(entry.member_id, item) - (entry.received[item.item_key] || 0) - alreadyAssigned;
...
async function fetchPreviousCantPayMembers(supabase, roundId) {
  ...
  const auctionIds = (auctionsResult.data || []).map((auction) => auction.id);
...
export async function startRound(supabase) {
  ...
  if (activeRound) throw new Error("There is already an active round.");

  const membersResult = await supabase.from("members").select(MEMBER_SELECT).order("char_name", { ascending: true });
  if (membersResult.error) throw membersResult.error;
  if (!membersResult.data?.length) throw new Error("Add members before starting an auction round.");
...
export async function startAuction(supabase, payload) {
  ...
  if (!activeRound) throw new Error("Start a round before starting an auction.");
  ...
  const allocationResult = buildAllocationRows({ context, inventoryByItemId });
  ...
  try {
    const queueRows = allocationResult.queue.map((entry) => ({
      member_id: entry.member_id,
      position: entry.position,
...
export async function markCantPay(supabase, memberId) {
  ...
  const allocationResult = buildAllocationRows({ context, inventoryByItemId, excludedMemberIds: excluded, queueMemberIds });
...
export async function finishActiveAuction(supabase) {
  ...
  const [allocationsResult, roundResult] = await Promise.all([
    supabase.from("auction_allocations").select("member_id,item_id,quantity").eq("auction_id", activeAuction.id),
  ...
  if (updates.length && updates.every((row) => row.is_complete)) {
    const roundUpdate = await supabase
      .from("rounds")
      .update({ status: "completed", completed_at: new Date().toISOString() })
      .eq("id", activeAuction.round_id);
...
export async function updateRoundLimits(supabase, payload) {
  ...
  if (!activeRound) throw new Error("Start a round before adjusting auction limits.");
