...
export async function startAuction(supabase, payload) {
  ...
  if (type === "league_prize") {
    ...
    orderedMemberIds = rotatedMemberIdsFromCursor(context, cantPayMemberIds, cooldownReferenceMs);
...
export async function cancelOpenAuctions(supabase, auctionIds = []) {
  ...
  if (!uniqueAuctionIds.length) return cancelOpenAuction(supabase);

  for (const auctionId of uniqueAuctionIds) {
    await cancelOpenAuction(supabase, auctionId);
  }
...
export async function markCantPay(supabase, memberId, auctionId = null) {
  ...
  const allocationResult = buildAllocationRows({
    context,
    inventoryByItemId,
    excludedMemberIds: excluded,
    queueMemberIds,
    orderedMemberIds,
    cooldownReferenceMs: timestampMs(activeAuction.started_at)
  });
...
export async function updateDefaultAuctionLimits(supabase, payload) {
  ...
  for (const item of itemsResult.data || []) {
    ...
    const { error } = await supabase
      .from("auction_items")
      .update({ default_per_round_cap: zeroOrPositiveInteger(caps[item.item_key]) })
      .eq("id", item.id);
    if (error) throw error;
  }
