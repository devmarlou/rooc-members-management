...
function compareNormalBeforePriority(context, a, b) {
  const aRank = auctionPriorityRank(context.membersById.get(a.member_id));
  const bRank = auctionPriorityRank(context.membersById.get(b.member_id));
...
function shouldCatchUpMemberForRound(member, round, nowMs = Date.now()) {
  return isMemberLateForRound(member, round) || isMemberInAuctionCooldown(member, nowMs);
}
...
function catchUpLateRoundProgress(progressRows, items, capResolver, membersById = new Map(), round = null, cooldownReferenceMs = Date.now()) {
  if (!capResolver) return;
  for (const item of items) {
    ...
    const eligibleRows = progressRows
      ...
      .filter((progress) => {
        const member = membersById.get(progress.member_id);
        return !member || !shouldCatchUpMemberForRound(member, round, cooldownReferenceMs);
      });
    ...
    for (const progress of progressRows) {
      const member = membersById.get(progress.member_id);
      const cap = capResolver.capFor(progress.member_id, item);
      if (!member || cap <= 0 || !shouldCatchUpMemberForRound(member, round, cooldownReferenceMs)) continue;

      const minimumHeldForCycle = completedCycles * cap;
      const heldCount = itemHeldCount(progress, item.item_key);
      const received = normalizeReceived(progress.received);
      const nextReceived = heldCount < minimumHeldForCycle
        ? addHeldItem(received, item.item_key, minimumHeldForCycle - heldCount)
        : received;
      nextReceived[item.item_key] = Math.min(
        Math.max(receivedHeldCount(nextReceived, item.item_key) - minimumHeldForCycle, 0),
        cap
      );
      progress.received = nextReceived;
    }
  }
}
...
async function fetchAuctionContext(supabase, round, auctionType = null) {
  ...
  catchUpLateRoundProgress([...progressByMemberId.values()], allItems, capResolver, membersById, round);
...
function buildAllocationRows({
  ...
  const compareCandidateOrder = (item) => (a, b) => {
    const prioritySort = !item
      ? 0
      : itemUsesAuctionPriority(item)
        ? compareAuctionPriority(context, a, b)
        : compareNormalBeforePriority(context, a, b);
  ...
  for (const item of context.items) {
    ...
    while ((remainingByItemId.get(item.id) || 0) > 0 && itemCandidates.length) {
      ...
      for (const entry of itemCandidates) {
        ...
        const cap = item.gates_round_completion ? context.capResolver.capFor(entry.member_id, item) : 1;
        let currentReceived = entry.received[item.item_key] || 0;
        let entryCycleTarget = resetCycleTarget;
        if (
          item.gates_round_completion &&
          itemUsesAuctionPriority(item) &&
          auctionPriorityRank(context.membersById.get(entry.member_id)) === 0 &&
          cap > 0 &&
          currentReceived >= cap &&
          alreadyAssigned === 0
        ) {
          entryCycleTarget = Math.max(resetCycleTarget, completedCyclesBeforeAuction + 1);
          currentReceived = 0;
        }
