...
function markReassignedAllocations(allocationRows, previousAllocations = []) {
  const signatureFor = (assignments = []) => JSON.stringify(
    assignments.map(({ reassigned, ...assignment }) => assignment)
  );
  const previousSignatures = new Map();
  for (const allocation of previousAllocations || []) {
    previousSignatures.set(`${allocation.member_id}:${allocation.item_id}`, signatureFor(allocation.page_assignments));
  }

  return allocationRows.map((row) => {
    const currentSignature = signatureFor(row.page_assignments);
    if (previousSignatures.get(`${row.member_id}:${row.item_id}`) === currentSignature) return row;
