...
function buildAllocationRows({
  ...
  // Walk the rotation in priority order and turn inventory into page/slot assignments.
  ...
  let candidates = context.rotation
    ...
    .sort((a, b) => {
      const prioritySort = compareAuctionPriority(context, a, b);
  ...
  for (const item of context.items) {
    ...
    while ((remainingByItemId.get(item.id) || 0) > 0 && candidates.length) {
      ...
      if (!gaveItemThisPass) {
        ...
        if (inGameCap > 0) {
          const hasInGameCapRoom = candidates.some((entry) => (
        ...
        candidates = [...candidates].sort((a, b) => compareAuctionPriority(context, a, b) || a.position - b.position);
      }
    }
  }
