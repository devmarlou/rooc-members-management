...
function resetItemProgress(progressRows, item) {
  for (const progress of progressRows) {
    progress.received = { ...normalizeReceived(progress.received), [item.item_key]: 0 };
  }
}

function cappedGatingItems(progressRows, items, capResolver) {
  return items.filter((item) => {
    if (!item.gates_round_completion) return false;
    const eligibleRows = progressRows.filter((progress) => capResolver.capFor(progress.member_id, item) > 0);
...
function applyAllocationsToContextProgress(context, allocations = []) {
  ...
  for (const unit of allocationUnits) {
    ...
    if (item.gates_round_completion && unit.item_cycle > (activeItemCycles.get(item.item_key) || 0)) {
      resetItemProgress(progressRows, item);
  ...
  for (const item of cappedGatingItems(progressRows, context.allItems, context.capResolver)) {
    resetItemProgress(progressRows, item);
  }
}
...
function markReassignedAllocations(allocationRows, previousAllocations = []) {
  ...
  for (const allocation of previousAllocations || []) {
    previousQuantities.set(`${allocation.member_id}:${allocation.item_id}`, allocation.quantity || 0);
  }

  return allocationRows.map((row) => {
    const previousQuantity = previousQuantities.get(`${row.member_id}:${row.item_id}`) || 0;
    const addedQuantity = Math.max(0, (row.quantity || 0) - previousQuantity);
    if (!addedQuantity) return row;

    let remaining = addedQuantity;
    const pageAssignments = [...row.page_assignments].reverse().map((assignment) => {
      if (remaining <= 0) return assignment;
      remaining -= 1;
      return { ...assignment, reassigned: true };
    }).reverse();

    return { ...row, page_assignments: pageAssignments };
  });
}
...
export async function getAuctionState(supabase) {
  ...
  const progressWithHeldTotals = progress.map((row) => ({
    ...row,
    held: heldTotalsByMember.get(row.member.id) || {}
  }));
...
export async function markCantPay(supabase, memberId, auctionId = null) {
  ...
  const previousAllocationsResult = await supabase
    .from("auction_allocations")
    .select("member_id,item_id,quantity")
    .eq("auction_id", activeAuction.id);
