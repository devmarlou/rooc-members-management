...
export async function startAuction(supabase, payload) {
  ...
  if (type === "league_prize") {
    ...
    if (!sourceGlAuction) throw new Error("Finalize the GL/WoE auction before starting League Prize.");
    ...
    if (latestLeagueAuction?.done_at && new Date(latestLeagueAuction.done_at) > new Date(sourceGlAuction.done_at)) {
      throw new Error("Create and finish another GL/WoE auction before starting League Prize again.");
    }
    ...
    const excludedFromGl = new Set((sourceQueueResult.data || [])
    ...
    orderedMemberIds = rotatedMemberIdsFromCursor(context, cantPayMemberIds)
      .filter((memberId) => excludedFromGl.has(memberId));
...
export async function finishActiveAuction(supabase, auctionId = null) {
  ...
  for (const allocation of allocationsResult.data || []) {
    ...
    const received = normalizeReceived(progress.received);
    const nextReceived = (received[item.item_key] || 0) + (allocation.quantity || 0);
    received[item.item_key] = item.gates_round_completion
      ? Math.min(nextReceived, context.capResolver.capFor(allocation.member_id, item))
      : nextReceived;
    progress.received = received;
  }
