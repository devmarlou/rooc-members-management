...
function resetGatedReceived(received, items) {
  const next = { ...received };
  for (const item of items) {
    if (item.gates_round_completion) next[item.item_key] = 0;
  }
  return next;
}
...
function buildAllocationRows({
  ...
  const candidates = context.rotation
    ...
    .map((entry) => {
      ...
      const actualReceived = normalizeReceived(progress?.received);
      const cycleReset = progress ? isMemberComplete(progress, context.allItems, context.capResolver) : false;
      const received = cycleReset ? resetGatedReceived(actualReceived, context.allItems) : actualReceived;
      const canReceive = context.items.some((item) => {
        if (!item.gates_round_completion) return true;
        return (received[item.item_key] || 0) < context.capResolver.capFor(entry.member_id, item);
      });
      return { ...entry, actualReceived, received, canReceive, cycleReset };
    })
    .filter((entry) => entry.canReceive)
    .sort((a, b) => {
      if (orderedMemberIds) return orderIndex.get(a.member_id) - orderIndex.get(b.member_id);
      const aPriority = priorityMemberIds.has(a.member_id) ? 0 : 1;
      const bPriority = priorityMemberIds.has(b.member_id) ? 0 : 1;
      return aPriority - bPriority || a.position - b.position;
    });
  ...
  for (const entry of candidates) {
    for (const item of context.items) {
      let remaining = remainingByItemId.get(item.id) || 0;
      const key = `${entry.member_id}:${item.id}`;
      const alreadyAssigned = assigned.get(key) || 0;
      const capacity = item.gates_round_completion
        ? context.capResolver.capFor(entry.member_id, item) - (entry.received[item.item_key] || 0) - alreadyAssigned
        : remaining;
      const quantityForMember = Math.min(remaining, capacity);
      ...
      for (let count = 0; count < quantityForMember; count += 1) {
        ...
        const unit = {
          ...
          slot: (pageIndex % 4) + 1,
          cycle_reset: Boolean(entry.cycleReset && item.gates_round_completion)
        };
        units.push(unit);
      }
...
export async function getAuctionState(supabase) {
  ...
  const activeAuction = await selectMaybeSingle(
    supabase
      .from("auctions")
  ...
  let activeAuctionState = null;
  if (activeAuction) {
    const [inventoryResult, queueResult, allocationsResult] = await Promise.all([
      supabase.from("auction_inventory").select("id,auction_id,item_id,quantity").eq("auction_id", activeAuction.id),
      supabase.from("auction_queue").select("id,auction_id,member_id,position,is_carry_over,status,removed_at").eq("auction_id", activeAuction.id).order("position", { ascending: true }),
      supabase.from("auction_allocations").select("id,auction_id,member_id,item_id,quantity,page_assignments,fulfilled").eq("auction_id", activeAuction.id)
    ]);
    ...
    activeAuctionState = {
      ...
      queue: (queueResult.data || []).map((row) => ({ ...row, member: context.membersById.get(row.member_id) || null })),
      allocations: allocationsResult.data || [],
      units,
      pageCount: units.reduce((max, unit) => Math.max(max, unit.page || 0), 0)
    };
  }
  ...
  const history = (historyResult.data || []).map((auction) => {
    ...
    return { ...auction, pageCount, allocatedCount };
  });
  ...
  return {
    ...
    progress,
    activeAuction: activeAuctionState,
    history,
    recentRounds: [],
    itemCaps
  };
...
export async function startAuction(supabase, payload) {
  ...
  const activeAuction = await selectMaybeSingle(
    supabase.from("auctions").select("id").eq("status", "active").limit(1)
  );
  if (activeAuction) throw new Error("Finish the active auction before starting another one.");
  ...
  const allocationResult = buildAllocationRows({ context, inventoryByItemId, priorityMemberIds: previousCantPay });
...
export async function markCantPay(supabase, memberId) {
  const activeAuction = await selectMaybeSingle(
    supabase.from("auctions").select("id,round_id,type,name,status,started_at,done_at").eq("status", "active").limit(1)
  );
  ...
  if (allocationResult.allocationRows.length) {
    const insertResult = await supabase
      .from("auction_allocations")
      .insert(allocationResult.allocationRows.map((row) => ({ ...row, auction_id: activeAuction.id })));
...
export async function recalculateActiveAuction(supabase) {
  const activeAuction = await selectMaybeSingle(
    supabase.from("auctions").select("id,round_id,type,name,status,started_at,done_at").eq("status", "active").limit(1)
  );
  if (!activeAuction) throw new Error("There is no active auction.");

  const [queueResult, inventoryResult] = await Promise.all([
    supabase
      .from("auction_queue")
      .select("id,member_id,position,status")
      .eq("auction_id", activeAuction.id)
...
export async function finishActiveAuction(supabase) {
  const activeAuction = await selectMaybeSingle(
    supabase.from("auctions").select("id,round_id,type,name,status,started_at,done_at").eq("status", "active").limit(1)
  );
  if (!activeAuction) throw new Error("There is no active auction.");
  ...
  const resetMemberIds = new Set();

  for (const allocation of allocationsResult.data || []) {
    if ((allocation.page_assignments || []).some((assignment) => assignment.cycle_reset)) {
      resetMemberIds.add(allocation.member_id);
    }
  }

  for (const memberId of resetMemberIds) {
    const progress = progressByMemberId.get(memberId);
    if (progress) progress.received = resetGatedReceived(normalizeReceived(progress.received), context.allItems);
  }

  for (const allocation of allocationsResult.data || []) {
    const item = itemsById.get(allocation.item_id);
    if (!item) continue;
    const progress = progressByMemberId.get(allocation.member_id);
    if (!progress) continue;
    const received = normalizeReceived(progress.received);
    received[item.item_key] = (received[item.item_key] || 0) + (allocation.quantity || 0);
