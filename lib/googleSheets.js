...
// Appends one row: [char name, title, link, recorded date, submitted at].
...
export async function appendPovLinkRow({ charName, title, link, recordedDate, submittedAt }) {
  if (!isConfigured()) {
  ...
  const response = await fetch(url, {
    ...
    body: JSON.stringify({
      values: [[charName, title, link, recordedDate, submittedAt]]
    })
  });
  if (!response.ok) {
