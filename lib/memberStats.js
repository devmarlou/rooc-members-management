...
// https:// when no scheme is present. Exported so both the write path
// (buildStatsRow) and read paths (rendering an already-stored/legacy link)
// can normalize the same way.
//
// Only http(s) is ever accepted as an existing scheme — anything else (most
// importantly "javascript:"/"data:"-style URIs, which the old any-scheme regex
// let straight through) is treated as schemeless and forced under https://
// instead, since this value is rendered verbatim as an <a href> in three
// places (account page, admin stats detail, public stats board) and a
// non-http(s) scheme there is a stored script-execution risk for whoever
// clicks the link, not just a bad link.
