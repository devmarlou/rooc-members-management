...
import { JOB_CLASS_SELECT, mapJobClassRow } from "@/lib/jobClasses";
...
export async function fetchBootstrapData(supabase, options = {}) {
  ...
  const [membersResult, groupsResult, itemsResult, auctionState, jobClassesResult] = await Promise.all([
  ...
  if (jobClassesResult.error) throw jobClassesResult.error;
  ...
  return {
    ...
    auctionState,
    jobClasses: (jobClassesResult.data || []).map(mapJobClassRow)
  };
}
